FR.initToolbar = function() {
	...
	FR.UI.actions = {
		toggleTree: new Ext.Button({
			iconCls: 'fa fa-folder-open fa-fw', cls: 'fr-btn-tree-toggle', enableToggle: true, hidden: !FR.isMobile,
			toggleHandler: function() {Ext.getCmp('FR-Tree-Region').toggleCollapse();}
		}),
		searchBtn: new Ext.Action({
			iconCls: 'fa fa-search fa-fw',
			listeners: {'afterrender': FR.UI.tooltip(FR.T('Search'))},
			handler: function() {
				FR.UI.searchPanel.doSearch();
			}
		}),
		...
		info: new Ext.Button({
			iconCls: 'fa fa-info-circle fa-fw', enableToggle: true, pressed: true,
		...
		profileSettings: new Ext.Action({
			text: FR.T('Account settings'), iconCls: 'fa-user', hidden: !User.perms.account_settings,
			handler: function() {FR.actions.openAccountSettings();}
		})
	};

	var newBtn = {
		text: (FR.isMobile ? false : FR.T('New')), iconCls: 'fa fa-plus icon-gray', cls: (FR.isMobile ? false : 'fr-btn-new'),
					...
					if (prevReqCheck) {
						...
						if (User.perms.share) {
							if (['sharedFolder', 'userWithShares'].indexOf(s) == -1) {
								if (this.location == 'tree') {
									rs = (!t.perms || (t.perms && t.perms.share));
								} else {
									rs = (!FR.currentFolderPerms || FR.currentFolderPerms.share);
								}
							}
