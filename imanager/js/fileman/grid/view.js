FR.components.gridView = Ext.extend(Ext.grid.GridView, {
	...
	refresh: function(headersToo) {
		...
		var np = FR.UI.explorerPanel.northPanel;
		if (this.viewMode != this.previousViewMode) {
			if (this.viewMode == 'music') {
				this.configureForAudioPlayer();
				np.getLayout().setActiveItem(0);
				np.expand();
				np.setHeight(138);
				FR.UI.explorerPanel.doLayout();
			} else {
				if (!np.collapsed) {
	...
	isListViewStyle: function() {
		return (['list', 'music', 'search'].indexOf(this.viewMode) != -1);
	}
});