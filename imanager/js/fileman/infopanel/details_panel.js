FR.components.detailsPanel = Ext.extend(Ext.Panel, {
	...
	showDetails: function() {
		...
		if (this.countSel == 1) {
			...
			if (FR.currentSection == 'search' && this.countSel == 0) {
				fields.push({name: FR.T('Search in'), values: [FR.utils.humanFilePath(FR.UI.searchPanel.params.searchPath)]});
			} else {return false;}
		}
		this.infoEl.show();
	...
	makeFieldset: function(set) {
		...
		if (set.name) {
			...
			if (set.id && !User.perms.read_only) {
				title.children.push({cls: 'value', children: [{tag: 'a', cls:'editIcon', html: '<li class="fa fa-edit"></li>'}]});
			}
			...
			if (field.values) {
				Ext.each(field.values, function (val) {
					if (field.type == 'large') {
						...
						val = {
							tag: 'div',
							html: '<a href="javascript:;" onclick="FR.utils.locateItem(\'' + pInfo.dirname + '\', \'' + pInfo.basename + '\')">' + FR.utils.humanFilePath(pInfo.dirname) + '</a>'
						};
					} else {
						if (field.id) {
							if (field.type == 'date') {
