FR.initTree = function() {
	var t = new Ext.tree.TreePanel({
		...
		enableDD: !User.perms.read_only, ddGroup: 'TreeDD', dropConfig: {appendOnly:true},
	...
	FR.UI.tree.searchResultsNode = r.appendChild(new tNode({
		text: FR.T('Search'), viewMode: 'search',
		iconCls: 'fa-search icon-gray',
		pathname: 'SEARCH', section: 'search', leaf: true, hidden: true,
	}));
	...
	t.getSelectionModel().on('selectionchange', function(sm, node) {
		...
		gridPanel.view.setMode(Ext.value(node.attributes.viewMode));
		gridPanel.reset();
		...
		if (['myfiles', 'sharedFolder'].indexOf(FR.currentSection) != -1) {
			FR.UI.actions.searchBtn.enable();
			...
			FR.UI.actions.newItem.disable();
			FR.UI.actions.searchBtn.disable();
